...
### TEST: Work out if the GDSC data is negative logged or not based on the data available in the online portal
```{r}
# A quick analysis. Define IC50s according to the values in the database portal for the top 5 LCML cell lines
IC50tests <- c("JURL-MK1" = 0.000313, "LAMA-84" = 0.000257, "MEG-01" = 0.000176, "KU812" = 0.000191, "CML-T1" = 0.000613)

# In the downloaded dataset, the column is titled "LN_IC50"
for(i in 1:length(IC50tests)){
  log(IC50tests[i]) %>% print(.)
}

IC50tests[i]

# Can use exp() to reverse log()
exp(-8.069)

# The calculated log values seem pretty close to the ones in the downloaded dataset, if not exactly the same. However, the AUC values are also a little bit different in this version compared to online. Compared to the latest version of the dataset available to download (27 Oct 23), they're identical to the one calculated by the log of the database portal value.

# Great! Looks like the GDSC is correctly labelled and these values are standard natural logs. You should make sure that no code incorrectly assumes that they aren't.
```
...
saveRDS(ensginPaths, file = "Data_TT/Ensembl_ID_in_Paths.Rds")
...
write.csv(SiggoseqPathways, file = "Data_TT/goseq_Das_Pathways.csv")
```
...
write.csv(expDat_sort, file = "Data_TT/BRCA_STAD_Das_Expression_Data.csv")#, row.names = rownames(expDat_sort))
...
* something similar can be achieved using something that collapses pathways, like the function in fgsea
```{r}
...
### WIP at the moment, to be finished after the rest of the code is fixed.
...
# pwf.c <- nullp(plotGenes.c, 'hg19', id = "knownGene", plot.fit = FALSE)
