...
### Load Data
## Note that GeneLabelTool doesn't pull from tt cause 1_TT doesn't save it
```{r, results = "hide", message = FALSE, warning = FALSE}
...
expDat_Dasatinib <- read.csv("Data_TT/BRCA_STAD_Das_Expression_Data.csv", row.names = 1)
## Note this doesn't pull from tt cause 1_TT doesn't save it
GeneLabelToolDasatinib <- read.csv("Data/Gene_Label_Tool.csv", row.names = 1)
goseqPathways <- read.csv("Data_TT/goseq_Das_Pathways.csv", row.names = 1)
ensginPaths <- readRDS("Data_TT/Ensembl_ID_in_Paths.Rds")

# Another alternative option: Run through with goseqPathways and ensginPaths filtered for significance
# names(ensginPaths) <- goseqPathways$Pathway
# goseqPathways <- goseqPathways[goseqPathways$adjP < 0.05,]
# ensginPaths <- ensginPaths[names(ensginPaths) %in% goseqPathways$Pathway]
...
# Ignore the in-chunk explanation, it's not really accurate. This still uses all pathways from goseq with adj-p-value < 1 (not just the ones statistically significant from goseq or gsea), but only clusters the ones with a correlation above the threshold (which is probably a low medium positive? )
...
png("Dasatinib_Metagenes_Output/Other/Pathway_Overlap.png", width = 3000, height = 3000)
...
for(p in 1:length(MetaPathways)){
  ...
  for(q in 1:length(M.SigDrugs)){
    if(length(M.SigDrugs > 0)){
      ...
      # Create heatmaps and save them as pngs.
      png(paste0("Dasatinib_Metagenes_Output/Metapathway_Heatmaps_TT/", mpn, "_", mdn, "_heatmap.png"), width = 5000, height = 4000)
...
<!--       # Create heatmaps and save them as pngs. -->
<!--       pdf(paste0("Dasatinib_Metagenes_Output/Metapathway_Heatmaps/", mpn, "_", mdn, "_heatmap.pdf"), width = 12, height = 10) -->
<!--       heatmap.mik(M.yy[ , M.colord], -->
<!--                   trace = 'none', -->
<!--                   scale = 'none', -->
<!--                   col = bluered(50), -->
...
<!--       # Create heatmaps and save them as pngs. -->
<!--       pdf(paste0("Dasatinib_Metagenes_Output/Metapathway_Heatmaps_No_MM/", mpn, "_", mdn, "_NO_MM_heatmap.pdf"), width = 12, height = 10) -->
<!--       heatmap.mik(T.yy[ , T.colord], -->
<!--                   trace = 'none', -->
<!--                   scale = 'none', -->
<!--                   col = bluered(50), -->
