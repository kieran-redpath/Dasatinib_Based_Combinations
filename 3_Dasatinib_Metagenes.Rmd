...
+ The strongest drug/pathway correlations (Spearman > 0.4 I think) were taken and plotted in heatmaps, to identify which genes in these pathways were over/under-expressed in cells that had increased response (high AUC). However, this created too many separate graphs to really be of use. In addition, only some of the pathways studied were statistically significantly linked to dasatinib.
...
# Makes some errors - might need to load other stuff in too, like Gene_Label_Tool
...
sum(abs(drugMetageneCorY)>0.4)
sum(abs(drugMetageneCorY)>0.5)

# Which rows have the most of these values, and more than 0?
PathDrugsNo <- rowSums(abs(drugMetageneCorY)>0.4) %>% .[.>0] %>% sort(., decreasing = TRUE) %>% as.data.frame(.)
colnames(PathDrugsNo) <- "No. of Significantly Associated Drugs"
...
DrugPathsNo <- colSums(abs(drugMetageneCorY)>0.4) %>% .[.>0] %>% sort(., decreasing = TRUE) %>% as.data.frame(.)
colnames(DrugPathsNo) <- "No. of Significantly Associated Reactome Pathways"
...
# Create a new matrix, replacing every value which isn't greater than abs(0.4) with an NA, to make it more clear which correlations are interesting.
SigDMCorY <- drugMetageneCorY
SigDMCorY[abs(SigDMCorY)<0.4] <- NA
...
# Make the loop only work on strong correlations (>0.4) between a drug and a pathway.
SigPaths <- rowSums(abs(drugMetageneCorY)>0.4) %>% sort(., decreasing = TRUE) %>% .[.>0]
```
...
for(p in 1:nrow(pathMatY)){
  ...
  if(rownames(pathMatY)[p] %in% names(SigPaths)){
    ...
    SigDrugs <- colSums(abs(aa)>0.4) %>% sort(., decreasing = TRUE) %>% .[.>0]
...
for(p in 1:length(MetaPathways)){
  ...
  M.SigDrugs <- colSums(abs(M.aa)>0.4) %>% sort(., decreasing = TRUE) %>% .[.>0]
...
for(p in 1:length(MetaPathways)){
  ...
  T.SigDrugs <- colSums(abs(T.aa)>0.4) %>% sort(., decreasing = TRUE) %>% .[.>0]
