...
sigfgseaRes <- read.csv("Data/fgsea_Das_Pathways.csv", row.names = 1)
sigfgseaResCollapsed <- read.csv("Data/fgsea_Das_Pathways_Collapsed.csv", row.names = 1)
goseqPathways <- read.csv("Data/goseq_Das_Pathways.csv", row.names = 1)

ensginPaths <- readRDS("Data/Ensembl_ID_in_Paths_fgsea.Rds")
...
write.csv(drugMetageneCorY, file = "Dasatinib_fgsea_Metagenes_Output/Other/Drug_Metagene_Correlations.csv", sep = ",", row.names = TRUE, col.names = TRUE)
```
...
kable(PathDrugsNo) %>% kable_styling(bootstrap_options = c("striped", "condensed", font_size=8), fixed_thead=T) %>% save_kable("Dasatinib_fgsea_Metagenes_Output/Other/PathDrugsNo.png")
...
kable(DrugPathsNo) %>% kable_styling(bootstrap_options = c("striped", "condensed", font_size=8), fixed_thead=T) %>% save_kable("Dasatinib_fgsea_Metagenes_Output/Other/DrugPathsNo.png")
...
png("Dasatinib_fgsea_Metagenes_Output/Other/Pathway_Overlap.png", width = 3000, height = 3000)
...
write.csv(M.drugMetageneCorY, file = "Dasatinib_fgsea_Metagenes_Output/Other/Drug_Metapathway_Correlations.csv", sep = ",", row.names = TRUE, col.names = TRUE)
```
...
for(p in 1:length(MetaPathways)){
  ...
  for(q in 1:length(M.SigDrugs)){
    if(length(M.SigDrugs > 0)){
      ...
      # Create heatmaps and save them as pngs.
      png(paste0("Dasatinib_fgsea_Metagenes_Output/Metapathway_Heatmaps/", mpn, "_", mdn, "_heatmap.png"), width = 5000, height = 4000)
