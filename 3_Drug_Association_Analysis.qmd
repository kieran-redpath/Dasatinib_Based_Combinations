...
# Which drugs satisfy this threshold with at least one pathway?
correlated_pathways <- rowSums(abs(drug_metagene_correlation) > cor_thresh) %>% .[. > 0] %>% sort(., decreasing = T)
correlated_pathways

correlated_drugs <- colSums(abs(drug_metagene_correlation) > cor_thresh) %>% .[. > 0] %>% sort(., decreasing = T)
correlated_drugs
