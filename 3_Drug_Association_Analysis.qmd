...
+ Carries out matrix multiplication to transform gene expression data for all cell lines based on the expression values in gastric and breast cancer cell lines with dasatinib response data.
+ Calculates Spearman correlations between pathway metagenes calculated with matrix multiplication and drug response across the entire set of cell lines shared between the GDSC2 and CCLE databases.
...
#     if(length(y) == 0){
#       drug_response[drug, cell_line] <- 0 # If there is no data for the combination of drug and cell_line, assign 0s.
#     } else{
...
### Isolate drug response data for each cell line
```{r}
# Extract drug names and IDs from GDSC2.
drug_info <- unique(GDSC2[, c("DRUG_ID", "DRUG_NAME")])

# Identify duplicated drug names.
duplicated_names <- drug_info$DRUG_NAME[duplicated(drug_info$DRUG_NAME)]
drug_info$rownames <- ifelse(
  drug_info$DRUG_NAME %in% duplicated_names,
  paste0(drug_info$DRUG_NAME, "_", drug_info$DRUG_ID),
  drug_info$DRUG_NAME
)

# Create response matrix.
drug_ids <- drug_info$DRUG_ID
cell_lines <- unique(GDSC2$CELL_LINE_NAME)
drug_response <- matrix(0, nrow = length(drug_ids), ncol = length(cell_lines))
rownames(drug_response) <- drug_info$rownames
colnames(drug_response) <- cell_lines

# Fill response matrix.
for (id in drug_ids) {
  rowname <- drug_info$rownames[drug_info$DRUG_ID == id]
  for (cell_line in cell_lines) {
    response <- GDSC2$AUC[GDSC2$DRUG_ID == id & GDSC2$CELL_LINE_NAME == cell_line]
    if(length(response) == 0){
      drug_response[rowname, cell_line] <- 0 # If there is no data for the combination of drug and cell_line, assign 0s.
    } else{
      drug_response[rowname, cell_line] <- response # In case there are multiple values for a drug and cell_line combination, take the first match.
    }
  }
}

# Tidy up the environment.
rm(list = c("id", "drug_ids", "rowname", "cell_lines", "cell_line", "response", "duplicated_names", "drug_info"))
```
