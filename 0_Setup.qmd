...
### Inspect GDSC2 data to extract total number of breast and gastric cancer cell lines
```{r}
unique_GDSC2 <- GDSC2 %>%
  dplyr::select(TCGA_DESC, CELL_LINE_NAME) %>%      # Keep only the desired columns
  distinct(CELL_LINE_NAME, .keep_all = TRUE) # Keep only the first row for each unique cell line

# Extract cell lines in the GDSC2 data that are BRCA or STAD according to TCGA classification.
TOI_GDSC2 <- unique_GDSC2 %>%
  dplyr::filter(TCGA_DESC %in% c("BRCA", "STAD"))

# Tidy up the environment.
rm(list = c("unique_GDSC2"))
```
...
# Create another tool for the same purpose, including breast and stomach cell lines regardless of dasatinib response data.
TOI_tissue_tool <- tissue_tool %>% filter(Tissue_Type %in% c("BREAST", "STOMACH")) # Data from CCLE.
common_lines <- intersect(TOI_GDSC2$CELL_LINE_NAME, TOI_tissue_tool$Cell_Line) # Data from GDSC2.
TOI_tissue_tool <- TOI_tissue_tool %>% # Overlap between the two.
...
# Tidy up the environment.
rm(list = c("split_cell_line_tissues", "common_lines", "TOI_GDSC2"))
```
...
# TOI_tissue_tool
saveRDS(TOI_tissue_tool, file = "data/TOI_tissue_tool.rds")
